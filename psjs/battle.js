...
function Side(battle,n){this.name='';this.id='';this.foe=null;this.spriteid=262;this.totalPokemon=6;this.x=0;this.y=0;this.z=0;this.missedPokemon=null;this.wisher=null;this.active=[null];this.lastPokemon=null;this.pokemon=[];this.sideConditions={};
this.battle=battle;
this.n=n;
this.updateSprites();
...
if(hitPokemon[0]!=='.'){for(var _i11=0;_i11<
hitPokemon.length;_i11++){var hitTarget=hitPokemon[_i11];
...
var _effect4=Tools.getEffect(args[3]);
...
case'-clearallboost':{for(var _i15=0,_this$sides4=
...
var _effect5=Tools.getEffect(args[2]);
...
this.scene.runOtherAnim('zpower',[_poke16]);
this.log(args,kwArgs);
break;
}
case'-prepare':{
...
var _target=this.getPokemon(args[3])||_poke17.side.foe.active[0]||_poke17;
this.scene.runPrepareAnim(moveid,_poke17,_target);
this.log(args,kwArgs);
break;
}
case'-mustrecharge':{
var _poke18=this.getPokemon(args[1]);
_poke18.addMovestatus('mustrecharge');
...
var _ofpoke5=this.getPokemon(kwArgs.of)||_poke19;
_poke19.status=args[2];
_poke19.removeVolatile('yawn');
this.activateAbility(_ofpoke5||_poke19,_effect6);
if(_effect6.effectType==='Item'){
_ofpoke5.item=_effect6.name;
}

switch(args[2]){
case'brn':
this.scene.resultAnim(_poke19,'Burned','brn');
this.scene.runStatusAnim('brn',[_poke19]);
break;
case'tox':
this.scene.resultAnim(_poke19,'Toxic poison','psn');
this.scene.runStatusAnim('psn',[_poke19]);
_poke19.statusData.toxicTurns=_effect6.name==="Toxic Orb"?-1:0;
break;
case'psn':
this.scene.resultAnim(_poke19,'Poisoned','psn');
this.scene.runStatusAnim('psn',[_poke19]);
break;
case'slp':
this.scene.resultAnim(_poke19,'Asleep','slp');
if(_effect6.id==='rest'){
_poke19.statusData.sleepTurns=0;
}
break;
case'par':
this.scene.resultAnim(_poke19,'Paralyzed','par');
this.scene.runStatusAnim('par',[_poke19]);
break;
case'frz':
this.scene.resultAnim(_poke19,'Frozen','frz');
this.scene.runStatusAnim('frz',[_poke19]);
break;
default:
...
var _effect7=Tools.getEffect(kwArgs.from);
...
switch(args[2]){
case'brn':
this.scene.resultAnim(_poke20,'Burn cured','good');
break;
case'tox':
case'psn':
_poke20.statusData.toxicTurns=0;
this.scene.resultAnim(_poke20,'Poison cured','good');
break;
case'slp':
this.scene.resultAnim(_poke20,'Woke up','good');
_poke20.statusData.sleepTurns=0;
break;
case'par':
this.scene.resultAnim(_poke20,'Paralysis cured','good');
break;
case'frz':
this.scene.resultAnim(_poke20,'Thawed','good');
break;
default:
_poke20.removeVolatile('confusion');
this.scene.resultAnim(_poke20,'Cured','good');}
...
case'-cureteam':{
var _poke21=this.getPokemon(args[1]);for(var _i17=0,_poke21$side$pokemon=
_poke21.side.pokemon;_i17<_poke21$side$pokemon.length;_i17++){var _target2=_poke21$side$pokemon[_i17];
_target2.status='';
this.scene.updateStatbarIfExists(_target2);
}

this.scene.resultAnim(_poke21,'Team Cured','good');
this.log(args,kwArgs);
break;
}
case'-item':{
...
_poke22.item=item.name;
_poke22.itemEffect='';
_poke22.removeVolatile('airballoon');
if(item.id==='airballoon')_poke22.addVolatile('airballoon');

if(_effect8.id){
switch(_effect8.id){
case'pickup':
this.activateAbility(_poke22,"Pickup");

case'recycle':
_poke22.itemEffect='found';
this.scene.resultAnim(_poke22,item.name,'neutral');
break;
case'frisk':
this.activateAbility(_ofpoke6,"Frisk");
if(_poke22&&_poke22!==_ofpoke6){
_poke22.itemEffect='frisked';
this.scene.resultAnim(_poke22,item.name,'neutral');
}
break;
case'magician':
case'pickpocket':
this.activateAbility(_poke22,_effect8.name);
...
_poke22.itemEffect='stolen';
this.scene.resultAnim(_poke22,item.name,'neutral');
this.scene.resultAnim(_ofpoke6,'Item Stolen','bad');
break;
case'harvest':
_poke22.itemEffect='harvested';
this.activateAbility(_poke22,"Harvest");
this.scene.resultAnim(_poke22,item.name,'neutral');
break;
case'bestow':
_poke22.itemEffect='bestowed';
this.scene.resultAnim(_poke22,item.name,'neutral');
break;
case'trick':
_poke22.itemEffect='tricked';
...
switch(item.id){
case'airballoon':
this.scene.resultAnim(_poke22,'Balloon','good');
break;}
...
var _effect9=Tools.getEffect(kwArgs.from);
_poke23.item='';
_poke23.itemEffect='';
_poke23.prevItem=_item2.name;
_poke23.prevItemEffect='';
_poke23.removeVolatile('airballoon');
_poke23.addVolatile('itemremoved');
if(kwArgs.eat){
_poke23.prevItemEffect='eaten';
this.scene.runOtherAnim('consume',[_poke23]);
...
switch(_effect9.id){
case'fling':
_poke23.prevItemEffect='flung';
break;
case'knockoff':
_poke23.prevItemEffect='knocked off';
this.scene.runOtherAnim('itemoff',[_poke23]);
this.scene.resultAnim(_poke23,'Item knocked off','neutral');
break;
case'stealeat':
_poke23.prevItemEffect='stolen';
break;
case'gem':
_poke23.prevItemEffect='consumed';
break;
case'incinerate':
_poke23.prevItemEffect='incinerated';
break;}

}else{
switch(_item2.id){
case'airballoon':
_poke23.prevItemEffect='popped';
_poke23.removeVolatile('airballoon');
this.scene.resultAnim(_poke23,'Balloon popped','neutral');
break;
case'focussash':
_poke23.prevItemEffect='consumed';
this.scene.resultAnim(_poke23,'Sash','neutral');
break;
case'focusband':
this.scene.resultAnim(_poke23,'Focus Band','neutral');
break;
case'redcard':
_poke23.prevItemEffect='held up';
break;
default:
_poke23.prevItemEffect='consumed';
break;}
...
var _effect10=Tools.getEffect(kwArgs.from);
var _ofpoke7=this.getPokemon(kwArgs.of);
_poke24.rememberAbility(ability.name,_effect10.id&&!kwArgs.fail);
...
switch(_effect10.id){
case'trace':
this.activateAbility(_poke24,"Trace");
this.scene.wait(500);
this.activateAbility(_poke24,ability.name,true);
_ofpoke7.rememberAbility(ability.name);
break;
case'powerofalchemy':
case'receiver':
this.activateAbility(_poke24,_effect10.name);
this.scene.wait(500);
this.activateAbility(_poke24,ability.name,true);
_ofpoke7.rememberAbility(ability.name);
break;
...
case'deltastream':
if(kwArgs.fail){
this.activateAbility(_poke24,ability.name);
}
break;
default:
this.activateAbility(_poke24,ability.name);
...
_poke25.ability='(suppressed)';

if(_ability.id){
if(!_poke25.baseAbility)_poke25.baseAbility=_ability.name;
}
this.log(args,kwArgs);
break;
}
...
_poke26.removeVolatile('formechange');
_poke26.removeVolatile('typeadd');
_poke26.removeVolatile('typechange');
...
if(level.charAt(0)==='L'){
_poke26.level=parseInt(level.substr(1),10);
}
newSpecies=args[2].substr(0,commaIndex);
}
var template=Tools.getTemplate(newSpecies);

_poke26.species=newSpecies;
_poke26.ability=_poke26.baseAbility=template.abilities?template.abilities['0']:'';
_poke26.weightkg=template.weightkg;

_poke26.details=args[2];
_poke26.searchid=args[1].substr(0,2)+args[1].substr(3)+'|'+args[2];

this.scene.animTransform(_poke26,true,true);
this.log(args,kwArgs);
			//mmo
...
if(_poke27===tpoke)throw new Error("Transforming into self");

if(!kwArgs.silent){
this.activateAbility(_poke27,_effect11);
}

_poke27.boosts=Object.assign({},tpoke.boosts);
_poke27.copyTypesFrom(tpoke);
_poke27.weightkg=tpoke.weightkg;
_poke27.ability=tpoke.ability;
...
_poke27.addVolatile('transform',pokemon,shiny,gender);
_poke27.addVolatile('formechange',species);for(var _i18=0,_tpoke$moveTrack=
tpoke.moveTrack;_i18<_tpoke$moveTrack.length;_i18++){var _trackedMove=_tpoke$moveTrack[_i18];
_poke27.rememberMove(_trackedMove[0],0);
}
this.scene.animTransform(_poke27);
this.scene.resultAnim(_poke27,'Transformed','good');
this.log(['-transform',args[1],args[2],tpoke.species],kwArgs);
break;
}
case'-formechange':{
var _poke28=this.getPokemon(args[1]);
var _template=Tools.getTemplate(args[2]);
var _fromeffect2=Tools.getEffect(kwArgs.from);
var isCustomAnim=false;
_poke28.removeVolatile('typeadd');
_poke28.removeVolatile('typechange');
if(this.gen>=7)_poke28.removeVolatile('autotomize');

if(!kwArgs.silent){
this.activateAbility(_poke28,_fromeffect2);
}
_poke28.addVolatile('formechange',_template.species);
this.scene.animTransform(_poke28,isCustomAnim);
this.log(args,kwArgs);
break;
}
case'-mega':{
...
var _effect12=Tools.getEffect(args[2]);
var _ofpoke8=this.getPokemon(kwArgs.of);
var _fromeffect3=Tools.getEffect(kwArgs.from);

this.activateAbility(_poke30,_effect12);
this.activateAbility(_ofpoke8||_poke30,_fromeffect3);
switch(_effect12.id){
case'typechange':
if(_ofpoke8&&_fromeffect3.id=='reflecttype'){
_poke30.copyTypesFrom(_ofpoke8);
}else{
var types=Tools.sanitizeName(args[3]||'???');
...
break;
case'typeadd':
var type=Tools.sanitizeName(args[3]);
_poke30.addVolatile('typeadd',type);
if(kwArgs.silent)break;
this.scene.typeAnim(_poke30,type);
break;
case'powertrick':
...
_poke30.addVolatile(_effect12.id);
this.scene.updateStatbar(_poke30);
this.log(args,kwArgs);
break;
}
...
var _effect13=Tools.getEffect(args[2]);
var _fromeffect4=Tools.getEffect(kwArgs.from);
_poke31.removeVolatile(_effect13.id);
...
break;
default:
if(_effect13.effectType==='Move'){
if(_effect13.name==='Doom Desire'){
this.scene.runOtherAnim('doomdesirehit',[_poke31]);
}
if(_effect13.name==='Future Sight'){
...
var _effect14=Tools.getEffect(args[2]);
_poke32.addTurnstatus(_effect14.id);
...
_poke32.rememberMove(_effect14.name,0);
break;
case'shelltrap':
...
var _effect15=Tools.getEffect(args[2]);
_poke33.addMovestatus(_effect15.id);
...
var _effect16=Tools.getEffect(args[2]);
var _ofpoke9=this.getPokemon(kwArgs.of);
this.activateAbility(_poke34,_effect16);
switch(_effect16.id){
case'grudge':
_poke34.rememberMove(args[3],Infinity);
break;
case'quickguard':
_poke34.addTurnstatus('quickguard');
this.scene.resultAnim(_poke34,'Quick Guard','good');
break;
case'wideguard':
_poke34.addTurnstatus('wideguard');
this.scene.resultAnim(_poke34,'Wide Guard','good');
break;
case'craftyshield':
_poke34.addTurnstatus('craftyshield');
this.scene.resultAnim(_poke34,'Crafty Shield','good');
break;
case'protect':
_poke34.addTurnstatus('protect');
...
break;
case'brickbreak':
_ofpoke9.side.removeSideCondition('Reflect');
_ofpoke9.side.removeSideCondition('LightScreen');
break;
case'hyperspacefury':
case'hyperspacehole':
case'phantomforce':
...
_poke34.removeTurnstatus('protect');for(var _i19=0,_poke34$side$pokemon=
_poke34.side.pokemon;_i19<_poke34$side$pokemon.length;_i19++){var _target3=_poke34$side$pokemon[_i19];
_target3.removeTurnstatus('wideguard');
_target3.removeTurnstatus('quickguard');
_target3.removeTurnstatus('craftyshield');
_target3.removeTurnstatus('matblock');
this.scene.updateStatbar(_target3);
}
break;
case'spite':
var move=Tools.getMove(args[3]).name;
var pp=Number(args[4]);
if(isNaN(pp))pp=4;
...
case'skillswap':
if(this.gen<=4)break;
var pokeability=Tools.sanitizeName(args[3])||_ofpoke9.ability;
var ofpokeability=Tools.sanitizeName(args[4])||_poke34.ability;
if(pokeability){
_poke34.ability=pokeability;
if(!_ofpoke9.baseAbility)_ofpoke9.baseAbility=pokeability;
}
if(ofpokeability){
_ofpoke9.ability=ofpokeability;
if(!_poke34.baseAbility)_poke34.baseAbility=ofpokeability;
}
if(_poke34.side!==_ofpoke9.side){
this.activateAbility(_poke34,pokeability,true);
this.activateAbility(_ofpoke9,ofpokeability,true);
}
break;


case'wonderguard':
this.scene.resultAnim(_poke34,'Immune','neutral');
break;
case'forewarn':
if(_ofpoke9){
_ofpoke9.rememberMove(args[3],0);
}else{
var foeActive=[];for(var _i20=0,_poke34$side$foe$acti=
_poke34.side.foe.active;_i20<_poke34$side$foe$acti.length;_i20++){var _target4=_poke34$side$foe$acti[_i20];if(_target4)foeActive.push(_target4);}
...
this.activateAbility(_ofpoke9,_ability2.name);
this.activateAbility(_poke34,"Mummy");
this.scene.wait(700);
this.activateAbility(_ofpoke9,"Mummy",true);
break;


case'leppaberry':
case'mysteryberry':
_poke34.rememberMove(args[3],_effect16.id==='leppaberry'?-10:-5);
break;
case'focusband':
_poke34.item='Focus Band';
break;
case'safetygoggles':
_poke34.item='Safety Goggles';
break;
case'protectivepads':
_poke34.item='Protective Pads';
break;
default:
if(kwArgs.broken){
this.scene.resultAnim(_poke34,'Protection broken','bad');
...
var _effect17=Tools.getEffect(args[2]);
_side.addSideCondition(_effect17);
...
var _effect18=Tools.getEffect(args[2]);


_side2.removeSideCondition(_effect18.name);
this.log(args,kwArgs);
break;
}
case'-weather':{
var _effect19=Tools.getEffect(args[1]);
var _poke35=this.getPokemon(kwArgs.of)||undefined;
var _ability3=Tools.getEffect(kwArgs.from);
if(!_effect19.id||_effect19.id==='none'){
kwArgs.from=this.weather;
}
this.changeWeather(_effect19.name,_poke35,!!kwArgs.upkeep,_ability3);
this.log(args,kwArgs);
break;
}
case'-fieldstart':{
var _effect20=Tools.getEffect(args[1]);
...
var maxTimeLeft=0;
if(['electricterrain','grassyterrain','mistyterrain','psychicterrain'].includes(_effect20.id)){
...
this.addPseudoWeather(_effect20.name,5,maxTimeLeft);
...
case'-fieldend':{
var _effect21=Tools.getEffect(args[1]);

this.removePseudoWeather(_effect21.name);
this.log(args,kwArgs);
break;
}
case'-fieldactivate':{
var _effect22=Tools.getEffect(args[1]);
switch(_effect22.id){
case'perishsong':
this.scene.updateStatbars();
break;}
...
kwArgs.silent='.';
this.useMove(_poke37,_move,_poke38,kwArgs);
...
_poke41.side.swapTo(_poke41,targetIndex,kwArgs);
}
this.log(args,kwArgs);
break;
}
...
if(!str)return;var _Battle$lineParse=
Battle.lineParse(str),args=_Battle$lineParse.args,kwArgs=_Battle$lineParse.kwArgs;
...
if(nextLine&&nextLine.substr(0,2)==='|-'){var _Battle$lineParse2=
Battle.lineParse(nextLine);nextArgs=_Battle$lineParse2.args;nextKwargs=_Battle$lineParse2.kwArgs;
}
