 ...
 */var
 ...
 Pokemon=function(){
	...
	var _item=Dex.getItem(move.isZ);
	if(_item.zMoveFrom)moveName=_item.zMoveFrom;
	}else if(move.name.slice(0,2)==='Z-'){
	moveName=moveName.slice(2);
	move=Dex.getMove(moveName);
	if(window.BattleItems){
	for(var _item2 in BattleItems){
	if(BattleItems[_item2].zMoveType===move.type)pokemon.item=_item2;
	}
	}
	}
	var pp=1;
	if(move.target==="all"){for(var _i11=0,_pokemon$side$foe$act=
	pokemon.side.foe.active;_i11<_pokemon$side$foe$act.length;_i11++){var _active=_pokemon$side$foe$act[_i11];
	if(_active&&toID(_active.ability)==='pressure'){
	pp+=1;
	}
	}
...
switch (args[0]) {
	...
	_stats.length;_i14++){var _stat4=_stats[_i14];
	...
	var _stats2=args[3]?args[3].split(', '):['atk','def','spa','spd','spe','accuracy','evasion'];for(var _i15=0;_i15<
	_stats2.length;_i15++){var _stat7=_stats2[_i15];
	...
	this.sides;_i16<_this$sides4.length;_i16++){var side=_this$sides4[_i16];for(var _i17=0,_side$active2=
	side.active;_i17<_side$active2.length;_i17++){var _active2=_side$active2[_i17];
	if(_active2){
	_active2.boosts={};
	this.scene.timeOffset=timeOffset;
	this.scene.resultAnim(_active2,'Stats reset','neutral');
	}
	}
	}
	...
	this.scene.resultAnim(_poke23,_item3.name,'neutral');
	break;
	case'frisk':
	...
	var _item4=Dex.getItem(args[2]);
	var _effect10=Dex.getEffect(kwArgs.from);
	_poke24.item='';
	_poke24.itemEffect='';
	_poke24.prevItem=_item4.name;
	...
	_poke25.rememberAbility(_ability.name,_effect11.id&&!kwArgs.fail);
	...
	this.scene.wait(500);
	this.activateAbility(_poke25,_ability.name,true);
	_ofpoke7.rememberAbility(_ability.name);
	break;
	case'powerofalchemy':
	case'receiver':
	...
	case'deltastream':
	if(kwArgs.fail){
	this.activateAbility(_poke25,_ability.name);
	}
	break;
	default:
	this.activateAbility(_poke25,_ability.name);
	...
	var _ability2=Dex.getAbility(args[2]);
	_poke26.ability='(suppressed)';
	
	if(_ability2.id){
	if(!_poke26.baseAbility)_poke26.baseAbility=_ability2.name;
	}
	this.log(args,kwArgs);
	break;
	}
	...
	if(commaIndex!==-1){
	var _level=newSpecies.substr(commaIndex+1).trim();
	if(_level.charAt(0)==='L'){
	_poke27.level=parseInt(_level.substr(1),10);
	}
	...
	var _species=tpoke.volatiles.formechange?tpoke.volatiles.formechange[1]:tpoke.species;
	var pokemon=tpoke;
	var _shiny=tpoke.shiny;
	var _gender=tpoke.gender;
	_poke28.addVolatile('transform',pokemon,_shiny,_gender);
	_poke28.addVolatile('formechange',_species);for(var _i19=0,_tpoke$moveTrack=
	...
	var _item5=Dex.getItem(args[3]);
	if(args[3]){
	_poke30.item=_item5.name;
	}
	this.log(args,kwArgs);
	break;
	}
	...
	var _ability3=Dex.getAbility(kwArgs.ability);
	this.activateAbility(_target3,_ability3.name);
	...
	this.changeWeather(_effect20.name,_poke36,!!kwArgs.upkeep,_ability4);
	this.log(args,kwArgs);
	break;
	}
	case'-fieldstart':{
	...
	this.sides;_i22<_this$sides5.length;_i22++){var _side3=_this$sides5[_i22];for(var _i23=0,_side3$active=
	_side3.active;_i23<_side3$active.length;_i23++){var _active3=_side3$active[_i23];
	if(_active3)this.scene.runOtherAnim('gravity',[_active3]);
	}
	}
	}
	break;}
	...
	var _active4=this.p1.active[slot];
	if(_active4&&_active4.searchid===searchid&&!isSwitch){
	_active4.slot=slot;
	return _active4;
	}
	...
	var _active5=this.p2.active[slot];
	if(_active5&&_active5.searchid===searchid&&!isSwitch){
	if(slot>=0)_active5.slot=slot;
	return _active5;
	}
