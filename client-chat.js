(function($) {
	...
	var ConsoleRoom = this.ConsoleRoom = Room.extend({
		...
		updateUser: function() {
			...
			var userid = vars.user.get('userid');
			if (this.expired) {
		...
		clickUsername: function(e) {
			...
			vars.addPopup(UserPopup, {name: name, sourceEl: e.currentTarget, position: position});
		},
		...
		openSounds: function() {
			vars.addPopup(SoundsPopup, {type:'semimodal'});
		},
		openOptions: function() {
			vars.addPopup(OptionsPopup, {type:'semimodal'});
		},
		...
		parseCommand: function(text) {
			...
			case 'challenge':
				...
				var challenge = function(targets) {
					...
					vars.on('response:userdetails', self.challengeUserdetails, self);
					vars.send('/cmd userdetails '+target);
				};

				if (!targets[0]) {
					vars.addPopupPrompt("Who would you like to challenge?", "Challenge user", function(target) {
						if (!target) return;
						challenge([target]);
					});
					return false;
			...
			case 'open':
				var open = function(target) {
					vars.addPopup(UserPopup, {name: target});
				};
				if (!target) {
					vars.addPopupPrompt("Username", "Open", function(target) {
						if (!target) return;
						open(target);
					});
					return false;
			...
			case 'unignore':
				...
				if (!vars.ignore[toUserid(target)]) {
					...
					delete vars.ignore[toUserid(target)];
			...
			case 'highlight':
				...
				if (target.indexOf(',') > -1) {
					...
					case 'add':
						...
						// We update the regex
						vars.highlightRegExp = new RegExp('\\b('+highlights.join('|')+')\\b', 'i');
						break;
					case 'delete':
						var newHls = [];
			...
			case 'rating':
			case 'ladder':
				if (!target) target = vars.user.get('userid');
				var self = this;
				$.get(vars.user.getActionPHP() + '?act=ladderget&user='+encodeURIComponent(target), Tools.safeJSON(function(data) {
					try {
						...
						if (!data.length) {
							...
							buffer += '<tr><th>Format</th><th><abbr title="Elo rating">Elo</abbr></th><th><abbr title="user\'s percentage chance of winning a random battle (aka GLIXARE)">GXE</abbr></th><th><abbr title="Glicko-1 rating: ratingÂ±deviation">Glicko-1</abbr></th><th>COIL</th><th>W</th><th>L</th><th>T</th></tr>';
			...
			case 'buttonban':
				var self = this;
				vars.addPopupPrompt("Why do you wish to ban this user?", "Ban user", function(reason) {
			...
			case 'buttonmute':
				var self = this;
				vars.addPopupPrompt("Why do you wish to mute this user?", "Mute user", function(reason) {
			...
			case 'buttonkick':
				var self = this;
				vars.addPopupPrompt("Why do you wish to kick this user?", "Kick user", function(reason) {
			...
			case 'join':
				...
				if (vars.rooms[target]) {
					vars.focusRoom(target);
					return false;
				}
				room = toId(target);
				if (vars.rooms[room]) {
		...
		challengeUserdetails: function (data) {
			vars.off('response:userdetails', this.challengeUserdetails);
			...
			vars.rooms[''].challenge(name, this.challengeData.format, this.challengeData.team);
		}
	});
	...
	var ChatRoom = this.ChatRoom = ConsoleRoom.extend({
		...
		join: function() {
			vars.send('/join '+this.id);
		},
		leave: function() {
			vars.send('/leave '+this.id);
		},
		...
		addChat: function(name, message, pm, deltatime) {
			...
			if (!pm && userid !== vars.user.get('userid')) {
			...
			var outputChat = function() {
				self.$chat.append(chatDiv + timestamp + '<strong style="' + color + '">' + clickableName + ':</strong> <em' + (name.substr(1) === vars.user.get('name') ? ' class="mine"' : '') + '>' + Tools.parseMessage(message, name) + '</em></div>');
			};
			var showme = !((Tools.prefs('chatformatting') || {}).hideme);
			if (pm) {
				var pmuserid = toUserid(pm);
				var oName = pm;
				if (pmuserid === vars.user.get('userid')) oName = name;
				...
				if (showme) {
					this.$chat.append(chatDiv + timestamp + '<strong style="' + color + '">&bull;</strong> <em' + (name.substr(1) === vars.user.get('name') ? ' class="mine"' : '') + '>' + clickableName + ' <i>' + Tools.parseMessage(message, name) + '</i></em></div>');
				} else {
					outputChat();
				}
